...
public enum CaptureError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .screenRecordingPermissionDenied:
            return tr("需要屏幕录制权限才能采集系统音频，请在系统设置→隐私与安全性→屏幕录制中允许 MeetingQ",
                      "Screen recording permission is required to capture system audio. Allow MeetingQ in System Settings → Privacy & Security → Screen Recording")
        case .noDisplay:
...
public final class AudioCaptureServiceImpl: AudioCaptureService {
    ...
    private let captureQueue = DispatchQueue(label: "meetingq.capture.sck", qos: .userInitiated)
