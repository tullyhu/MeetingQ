...
public struct OnboardingView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 14) {
            ...
            Text(tr("欢迎使用 MeetingQ", "Welcome to MeetingQ"))
                .font(.title.weight(.bold))
            Text(tr(
                "MeetingQ 会在会议中实时转写内容，并在有人叫到你名字时立刻提醒你。\n开始前需要授予以下权限：",
                "MeetingQ transcribes your meetings live and alerts you the moment someone says your name.\nTo get started, please grant the following permissions:"))
                .font(.callout)
    ...
    private var identityStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(tr(
                "当会议中有人叫到你的名字时，MeetingQ 会弹窗提醒你并附上当时的截图与上下文。多个名字用逗号分隔。",
                "When someone says your name in a meeting, MeetingQ pops up an alert with a screenshot and the surrounding context. Separate multiple names with commas."))
                .font(.callout)
                .foregroundStyle(.secondary)
                .lineSpacing(5)
            ...
            Label(tr("语音识别偶尔会把名字听成同音字，MeetingQ 会自动学习这些变体。",
                     "Speech recognition sometimes mishears names as homophones — MeetingQ learns these variants automatically."),
    ...
    private var llmStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Label(tr("配置 AI 模型", "Configure Your AI Model"), systemImage: "brain")
                .font(.title2.weight(.bold))
            Text(tr(
                "MeetingQ 使用 OpenAI 兼容接口生成摘要和检测议题。推荐在本机通过 oMLX 运行 Qwen3.6 系列多模态大模型——同时理解转写文本与会议截图，零费用、全离线。也可使用任意云端服务。",
                "MeetingQ uses any OpenAI-compatible API for summaries and topic detection. We recommend running a Qwen3.6-series multimodal model locally via oMLX — it understands both transcripts and meeting screenshots, free and fully offline. Cloud services work too."))
                .font(.callout)
                .foregroundStyle(.secondary)
                .lineSpacing(5)
    ...
    private var nextTitle: String {
        ...
        default: return tr("开始使用", "Start Using MeetingQ")
        }
    }
