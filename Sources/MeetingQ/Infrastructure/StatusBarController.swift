...
public final class StatusBarController: NSObject, NSMenuDelegate {
    ...
    public init(icon: NSImage? = nil) {
        ...
        super.init()
        let image = icon ?? NSImage(systemSymbolName: "waveform.circle.fill", accessibilityDescription: "MeetingQ")
    ...
    public func setMonitoring(_ isMonitoring: Bool) {
        ...
        let image = NSImage(systemSymbolName: name, accessibilityDescription: "MeetingQ")
        image?.isTemplate = true
        statusItem.button?.image = image
    }
    ...
    private func buildMenu() {
        ...
        menu.addItem(makeItem(tr("退出 MeetingQ", "Quit MeetingQ"), action: #selector(quit), keyEquivalent: "q"))
